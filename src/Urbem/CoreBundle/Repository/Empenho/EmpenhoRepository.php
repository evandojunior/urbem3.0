...
class EmpenhoRepository extends ORM\EntityRepository
{
    ...
    public function filterEmpenhoLiquidacao($filter)
    {
        ...
        select
            tabela.cod_nota,
	            tabela.exercicio_nota,
	            to_char(tabela.dt_liquidacao, 'dd/mm/yyyy') as dt_liquidacao,
	            to_char(tabela.dt_vencimento_liquidacao, 'dd/mm/yyyy') as dt_vencimento_liquidacao,
	            tabela.cod_autorizacao,
	            tabela.cod_empenho,
	            to_char(tabela.dt_vencimento, 'dd/mm/yyyy') as dt_vencimento,
	            to_char(tabela.dt_empenho, 'dd/mm/yyyy') as dt_empenho,
	            tabela.descricao,
	            tabela.exercicio,
	            tabela.cod_pre_empenho,
	            tabela.implantado,
        ...
        from
            (
                select
                    ...
                    empenho.fn_consultar_valor_liquidado_nota(
                        tabela.exercicio_nota,
                        tabela.cod_empenho,
                        tabela.cod_entidade,
                        tabela.cod_nota
                    ) as vl_liquidado,
                    empenho.fn_consultar_valor_liquidado_anulado_nota(
                        tabela.exercicio_nota,
                        tabela.cod_empenho,
                        tabela.cod_entidade,
                        tabela.cod_nota
                    ) as vl_liquidado_anulado,
                    empenho.fn_consultar_valor_pagamento_nota(
                        tabela.exercicio_nota,
                        tabela.cod_nota,
                        tabela.cod_entidade
                    ) as vl_pago,
                    empenho.fn_consultar_valor_pagamento_anulado_nota(
                        tabela.exercicio_nota,
                        tabela.cod_nota,
                        tabela.cod_entidade
                    ) as vl_pago_anulado
                from
                    (
                        select
                            ...
                            NL.dt_vencimento as dt_vencimento_liquidacao,
                            AE.cod_autorizacao,
                            EE.cod_empenho,
                            EE.dt_vencimento as dt_vencimento,
                            ...
                            AR.cod_reserva,
                            C.nom_cgm as nom_fornecedor,
                            C.numcgm as num_fornecedor,
                            R.vl_reserva
                        from
                            empenho.empenho as EE left join empenho.empenho_autorizacao as EA on
                            (
                                EA.exercicio = EE.exercicio
                                and EA.cod_entidade = EE.cod_entidade
                                and EA.cod_empenho = EE.cod_empenho
                            ) left join empenho.autorizacao_empenho as AE on
                            (
                                AE.exercicio = EA.exercicio
                                and AE.cod_autorizacao = EA.cod_autorizacao
                                and AE.cod_entidade = EA.cod_entidade
                            ) left join empenho.autorizacao_reserva as AR on
                            (
                                AR.exercicio = AE.exercicio
                                and AR.cod_entidade = AE.cod_entidade
                                and AR.cod_autorizacao = AE.cod_autorizacao
                            ) left join orcamento.reserva as R on
                            (
