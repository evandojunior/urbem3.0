...
class OrgaoRepository extends ORM\EntityRepository
{
    public function getProximoValorByCodOrgao($codOrganograma, $codOrgao, $codNivel)
    {
        ...
        $sql = "SELECT valor FROM organograma.orgao_nivel WHERE cod_nivel = ". $codNivel;
        $sql .= " AND cod_orgao = (";
        $sql .= "SELECT MAX(cod_orgao) FROM organograma.vw_orgao_nivel where orgao like :orgao ";
        $sql .= "AND cod_organograma = " . $codOrganograma . " AND nivel = " . $orderNivel[$codNivel];
        $sql .= ")";
        ...
        $valor = 1;
        foreach ($orgaoNivel as $value) {
            $valor = ((int) $value["valor"]) + 1;
        }
