$(document).ready(function() {
    ...
    //Desabilita o form
    disableForm(tooltTipConsideracaoExecucaoVariacao);

    /*Escuta botão de busca*/
    $("#btnSearch").on("click", function() {
        buscaParametros();
        enableForm(tooltTipConsideracaoExecucaoVariacao);
    });

    // Add Tooltip on Inputs
    $.each(tooltTipConsideracaoExecucaoVariacao, function(index, value) {
    ...
    function buscaParametros() {
        ...
        $.post(UrlServiceProviderTCE, $("form").serializeArray())
            .success(function (data) {
                    ...
                    if (count <= 1) {
                        clearInputs(tooltTipConsideracaoExecucaoVariacao);
                    }
                    modal.close();
                }
            );
