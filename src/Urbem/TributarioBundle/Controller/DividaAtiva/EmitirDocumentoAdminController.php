...
class EmitirDocumentoAdminController extends CRUDController
{
    ...
    public function apiDocumentoAction(Request $request)
    {
        ...
        $qb = $em->getRepository(Documento::class)->createQueryBuilder('d');
        ...
        $qb->join(ModeloDocumento::class, 'md', 'WITH', 'md.codDocumento = d.codDocumento AND md.codTipoDocumento = d.codTipoDocumento');

        $qb->where('(s.numcgm = :numcgm OR LOWER(s.nomCgm) LIKE LOWER(:nomCgm))');
        $qb->setParameter('numcgm', (int) $request->get('q'));
        $qb->setParameter('nomCgm', sprintf('%%%s%%', $request->get('q')));

        $qb->orderBy('s.numcgm', 'ASC');

        foreach ((array) $qb->getQuery()->getResult() as $documento) {
            $modeloDocumento = $documento->getFkAdministracaoModeloDocumento();
            $dividaParcelamento = $documento->getFkDividaParcelamento()->getFkDividaDividaParcelamentos()->last();
            $dividaCgm = $dividaParcelamento->getFkDividaDividaAtiva()->getFkDividaDividaCgns()->last();

            $results['items'][] = [
                'id' => sprintf(
                    '%d~%d~%d',
                    $documento->getNumParcelamento(),
                    $documento->getFkDividaEmissaoDocumentos()->last()->getNumDocumento(),
                    $documento->getFkDividaEmissaoDocumentos()->last()->getExercicio()
                ),
                'label' => sprintf(
                    '%d - %s - %d/%d - %s',
                    $modeloDocumento->getCodTipoDocumento(),
                    $modeloDocumento->getNomeDocumento(),
                    $dividaParcelamento->getCodInscricao(),
                    $dividaParcelamento->getExercicio(),
                    $dividaCgm->getFkSwCgm()
                ),
            ];
        }
