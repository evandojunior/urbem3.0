...
class DepositoAdmin extends AbstractSonataAdmin
{
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        ...
        $formMapper
            ...
            ->end()
            ->with('Dados para depósito', ['class' => 'dados-arrecadacao'])
            ->add(
                'contaPlanoCredito',
                'autocomplete',
                [
    ...
    public function validate(ErrorElement $errorElement, $object)
    {
        ...
        if ($valor <= 0) {
            $error = $this->getTranslator()->trans("label.tesouraria.aplicacoes.validacoes.campoMaiorZero");
            $this->getRequest()->getSession()->getFlashBag()->add("erro_custom", $error);
        }

        if ($contaDebito == $contaCredito) {
            $error = $this->getTranslator()->trans("label.tesouraria.aplicacoes.validacoes.campoSimilar");
            $this->getRequest()->getSession()->getFlashBag()->add("erro_custom", $error);
        }
    ...
    public function prePersist($object)
    {
        ...
        try {
            ...
            $container->get('session')->getFlashBag()->add('success', "Depósito realizado");
            $this->forceRedirect(
                "/financeiro/tesouraria/deposito/list"
            );
        } catch (\Exception $e) {
            $container->get('session')->getFlashBag()->add('error', Exception\Error::ERROR_PERSIST_DATA);
            $this->forceRedirect(
                "/financeiro/tesouraria/deposito/create"
            );
        }
    }
}
