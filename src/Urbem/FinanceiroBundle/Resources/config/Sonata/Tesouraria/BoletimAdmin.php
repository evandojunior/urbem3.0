...
class BoletimAdmin extends AbstractAdmin
{
    ...
    public function validate(ErrorElement $errorElement, $object)
    {
        ...
        if ($repository->isExercicioFoiEncerrado($this->getExercicio())) {
        ...
        if ($this->getExercicio() != $this->getExercicio()) {
            $this->validateDtBoletim(
                $errorElement,
                $this->getContainer()->get('translator')->transChoice('label.tesouraria.boletim.validacoes.dataExercicioAtual', 0, [], 'messages')
            );
        }
        ...
        if ($repository->findOneBy(['dtBoletim' => $date, 'codEntidade' => $codEntidade, 'exercicio' => $this->getExercicio()])) {
            $this->validateDtBoletim(
                $errorElement,
                $this->getContainer()->get('translator')->transChoice('label.tesouraria.boletim.validacoes.dataDuplicada', 0, [], 'messages')
            );
        }
    ...
    protected function configureFormFields(FormMapper $formMapper)
    {
        ...
        $lastBoletim = $repository->findOneBy([], ['dtBoletim' => 'DESC','codBoletim' => 'DESC']);
