...
class PlanoContaAdmin extends AbstractSonataAdmin
{
    ...
    public function validate(ErrorElement $errorElement, $object)
    {
        ...
        if (!empty($object->getIndicadorSuperavit())) {
            $planoContaRepository = $this->getDoctrine()->getRepository(PlanoConta::class);

            if (preg_replace('/.*(::)/', '', $this->getRequest()->attributes->get('_controller')) != self::ACTION_EDIT) {
                $dadosBuscaPlanoConta = ['codEstrutural' => $object->getCodEstrutural(), 'indicadorSuperavit' => $object->getIndicadorSuperavit(), 'exercicio' => $this->getExercicio()];
                $planoConta = $planoContaRepository->findOneBy($dadosBuscaPlanoConta);
                if (!empty($planoConta)) {
                    $this->validatePlanoConta(
                        $errorElement,
                        $this->getContainer()->get('translator')->transChoice('label.planoconta.validacoes.naoUnico', 0, $dadosBuscaPlanoConta, 'messages'),
                        "indicadorSuperavit"
                    );
                }
            }

            $codSistemaExercicio = explode('-', $formData['codSistema']);
            $codSistema = current($codSistemaExercicio);
            $exercicio  = next($codSistemaExercicio);
            $object->setCodSistema($codSistema);
            $validacao = $planoContaRepository->validarVerificadoSuperavit($codSistema, $exercicio);
            if (empty($validacao)) {
                $this->validatePlanoConta(
                    $errorElement,
                    $this->getContainer()->get('translator')->transChoice('label.planoconta.validacoes.validaIndicadorSuperavitSistemaContabil', 0, ['indicadorSuperavit' => $object->getIndicadorSuperavit(), 'codSistema'=> $codSistema], 'messages'),
                    "indicadorSuperavit"
                );
            } else {
                // Contas do Grupo 4 só podem ser de natureza 'Credor'
                if (in_array(4, explode(',', current($validacao)['grupos']))) {
                    if ($object->getNaturezaSaldo() != 'credor') {
                        $this->validatePlanoConta(
                            $errorElement,
                            $this->getContainer()->get('translator')->transChoice('label.planoconta.validacoes.validaGrupoQuatro', 0, [], 'messages'),
                            "naturezaSaldo"
                        );
                    }
                }
        ...
        // Verifica se o código de classificação existe e o admin é create. Caso sim apresenta erro.
        $codEstrutural = $this->getDoctrine()->getRepository(PlanoConta::class)->findByCodEstrutural($object->getCodEstrutural());

        $isCreate = $this->isCurrentRoute('create');
        if ($codEstrutural && $isCreate) {
            $mensagem =  $this->getTranslator()->trans('label.planoconta.validacoes.erroSalvar');
            $this->validatePlanoConta($errorElement, $this->getContainer()->get('translator')->transChoice($mensagem, 0, $codEstrutural, 'messages'), "codEstrutural");
        }
    }
     ...
     * @param $error
     */
    public function validatePlanoConta(ErrorElement $errorElement, $error, $with)
    {
    ...
    public function preRemove($object)
    {
        ...
        // Mostra erro caso tentar excluir conta pai.
        if ($this->canRemove($object, ['fkContabilidadeClassificacaoPlanos'])) {
            return true;
        }
